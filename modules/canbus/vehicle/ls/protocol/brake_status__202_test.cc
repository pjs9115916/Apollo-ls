...
namespace canbus {
namespace ls {
class Brakestatus202Test : public ::testing::Test {
 public:
  virtual void SetUp() {}
};

TEST_F(Brakestatus202Test, General) {
  uint8_t data[8] = {0x01, 0x02, 0x03, 0x04, 0x11, 0x12, 0x13, 0x14};
  int32_t length = 8;
  ChassisDetail cd;
  Brakestatus202 brake;
  ...
  EXPECT_EQ(cd.ls().brake_status__202().brake_pedal_en_sts(), 1);
  EXPECT_EQ(cd.ls().brake_status__202().brake_pedal_sts(), 2);
}
